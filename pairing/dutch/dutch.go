...
func (p *Pairer) Pair(_ context.Context, state *chesspairing.TournamentState) (*chesspairing.PairingResult, error) {
	...
	// The blossom matching leaves players unmatched when no legal pairing
	// covers the field; report that rather than returning a round with
	// players missing from it.
	if err := swisslib.ValidateResult(players, preAssignedByes, result); err != nil {
		return nil, err
	}
